...
export function SurveyCard({ initialAddress, brand }: SurveyCardProps) {
  ...
  // Omega: excellent = SOFT-DQ (submits as LeadLowIntent, not hard-blocked). Default off = v2's
  // baked hard-DQ, unchanged for other clients.
  ...
  const handleOptionSelect = (field: keyof SurveyData, value: string) => {
    ...
    if (field === "condition" && value === "excellent" && !excellentPass && !excellentSoftDq) {
